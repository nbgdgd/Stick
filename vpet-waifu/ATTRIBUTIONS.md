...
(формат и порядок установки — `docs/sprite-packs.md`). Наборы **не входят в
репозиторий и не поставляются с приложением**: каталог `app/src/main/assets/pets/`
пуст, и по умолчанию рисуется собственный векторный персонаж.

Любой набор, добавленный туда, попадёт в сборку. Перед публикацией релиза
проверьте, что на каждый лежащий там набор есть право на коммерческое
распространение: персонаж из изданного аниме или игры принадлежит студии,
а не сайту, с которого лист скачан.
