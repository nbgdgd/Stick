...
### Anya — права не наши, коммерческое распространение не разрешено

С версии 1.5.0 набор `app/src/main/assets/pets/anya/` **лежит в репозитории**
и попадает в отладочные и preview-сборки. Это персонаж из изданного
произведения (Spy × Family, SPY×FAMILY © Tatsuya Endo / Shueisha); права
принадлежат правообладателю, а не проекту, и лицензии на коммерческое
распространение у проекта нет.

Поэтому `packageRelease` и `bundleRelease` **отказываются собираться**, пока
набор лежит в дереве, и предлагают два выхода: убрать папку или передать
`-PbundlePacks=1`. Второе — осознанное решение опубликовать чужого персонажа,
и принимать его должен человек, а не сборка.

Любой другой набор, добавленный в тот же каталог, ведёт себя так же. Перед
публикацией проверьте, что на него есть право на коммерческое распространение:
персонаж из изданного аниме или игры принадлежит студии, а не сайту, с которого
скачан лист.
