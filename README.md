...
Reddit ограничивает частоту: два фида подряд с одного IP регулярно ловят 429.
Поэтому между фидами пауза 1,5 с и один повтор через 3 с; если и он упирается в
лимит, обновление откладывается до следующего запуска, а на экране остаются
сохранённые предложения.

**Обновление не падает целиком.** Каждый этап изолирован: упавший источник или
сбойная запись не отменяют то, что уже успели получить остальные. Причина сбоя
не проглатывается — она попадает в лог и в сообщение на экране («Обновлено
частично — каталог: HTTP 404»), потому что «не удалось обновить» без причины
нельзя ни понять, ни починить. Отмена корутины (пользователь ушёл с экрана,
WorkManager снял задачу) больше не считается ошибкой — раньше она показывалась
как неудачное обновление. Поведение закреплено тестами
(`data/DealsRepositoryRefreshTest.kt`).
...
**Покрытие — 29 из 80** сервисов базы подтверждаются автоматически. Остальные
...
**80 сервисов**: 43 предложения (37 триалов + 6 скидок) и 37 сервисов в
`watchlist` — тех, у кого предложение пока не подтверждено.

Каталог **генерируется**, а не пишется руками:

```bash
python3 tools/build_catalog.py          # 80 сервисов, ~2 минуты
```

Скрипт ходит в листинг Play за названием, описанием и иконкой, в страницу
тарифов — за сроком триала, и пересобирает `catalog/deals.json` вместе с копией
в assets. Правила извлечения те же, что в приложении — при изменении одного
надо править и второе. Добавить сервис = четыре поля в `tools/services.json` и
перезапуск. Ручные записи при этом сохраняются: скрипт всегда мержится поверх
закоммиченного каталога и перезаписывает только то, что смог подтвердить.
...
python3 tools/build_catalog.py          # пересобрать каталог из tools/services.json
```
