...
Приложение берёт предложения из двух источников — они дополняют друг друга.

#### 2a. Живой парсинг публичных фидов (скидки)

Единого API с триалами не существует, но **скидки Google Play публикуются
непрерывно**, и Reddit отдаёт каждый сабреддит как Atom-фид. Это документированный
...
#### 2b. Каталог пробных подписок (триалы)

Триалы так получить нельзя — машиночитаемого источника с ними не существует,
...
`brand_color`, `icon_url`, `glyph`, `popularity`.
...
* **Настройки** — периодичность проверки, системные приложения, приватность,
  статус каждого источника (сколько предложений он дал в последний раз).
...
data/
 ...
 ├ remote/     CatalogRemoteSource · DealFeedSource (OkHttp)
 ├ parse/      AtomFeedParser · DealTitleParser · RedditDealMapper
 ├ PackageScanner.kt
